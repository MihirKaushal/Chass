      ...
      <div className="studio-shell">
        ...
        <div className="studio-controls">
          ...
          <section className="studio-section gambit-config-section">
            ...
            <Toggle checked={draft.gambit.enabled} onChange={(enabled) => setDraft((current) => ({ ...current, presetId: enabled ? "gambit" : "custom", formationId: enabled ? "classic" : "custom", gambit: { ...current.gambit, enabled } }))} label="Enable Chass Gambit" description="Each player edits only their closest home rows and must spend the full budget." />
            {draft.gambit.enabled ? <div className="gambit-settings-grid">
              <label>Points Available<input type="number" min="0" value={draft.gambit.budget} onChange={(event) => setDraft((current) => ({ ...current, presetId: "custom", gambit: { ...current.gambit, budget: Math.max(0, Number(event.target.value)) } }))} /><small>Players must spend exactly this amount. Debt is not allowed.</small></label>
