...
Backend and Firestore package versions are pinned in `backend/requirements.txt`, and
Render's Python version is pinned in `.python-version`. Update these deliberately and run
the Firestore repository tests plus a deployed create/load smoke test before accepting a
new database-client release. The dependency contract also verifies that the special
`(default)` Firestore database ID remains literal rather than URL-encoded in resource paths.
